 ...
 * storage account - Update storage account - List storage
...
public final class ManageStorageAccount {
    ...
    public static boolean runSample(Azure azure, String location) {
        ...
        try {
            ...
            List<StorageAccountKey> storageAccountKeys = azure.storageAccounts().listKeysAsync(rgName, storageAccountName)
                    .toList().toBlocking().last().get(0).keys();

            Utils.print(storageAccountKeys);

            System.out.println("Regenerating first storage account access key");

            storageAccountKeys = azure.storageAccounts().regenerateKeyAsync(rgName, storageAccountName, storageAccountKeys.get(0).keyName())
                    .toList().toBlocking().last().get(0).keys();
            ...
            Encryption encryptSettings = new Encryption()
                    .withServices(new EncryptionServices()
                    .withBlob(new EncryptionService()));
        
            storageAccount2.update()
                    .withEncryption(encryptSettings)
                    .apply();
